...
我们通常会使用折线图来分析趋势，但是有的时候有需要明确的知道某个数据是否在某个区间范围内，因此我们可以主动地去配置定义区域。例如我们设定 25%-75% 这个范围内为我们计划完成的销售额，那我们可以再这个定义区域的范围内，给每个区域设置成一个底色，这样就可以高效的识别出每个数据处于哪个区间范围内，未达到、达到、超额完成的值。
